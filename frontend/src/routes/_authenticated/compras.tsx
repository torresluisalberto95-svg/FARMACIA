...
function ComprasPage() {
  ...
  const eliminarTodas = async () => {
    setEliminando(true);
    try {
      const msg = await comprasApi.eliminarTodas();
      toast.success(msg ?? "Todas las compras eliminadas");
      load();
    } catch (err: any) {
      toast.error(err.response?.data ?? "Error al eliminar compras");
    } finally {
      setEliminando(false);
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card className="p-5">
          ...
          <div className="mt-4 grid grid-cols-2 lg:grid-cols-[1fr_80px_120px_auto] gap-2 items-end">
      ...
      {/* Diálogo: Eliminar todas las compras */}
      <Dialog open={confirmEliminar} onOpenChange={v => !v && setConfirmEliminar(false)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Se eliminarán <strong>todas las compras</strong> del historial y el contador se reiniciará desde 1. Después podrás eliminar los productos del inventario. Esta acción <strong>no se puede deshacer</strong>.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2">
            <Button variant="outline" className="flex-1" onClick={() => setConfirmEliminar(false)}>
              Cancelar
            </Button>
            <Button variant="destructive" className="flex-1" disabled={eliminando} onClick={eliminarTodas}>
